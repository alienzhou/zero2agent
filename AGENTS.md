...
在做功能设计时，不只关注“能不能跑通”，也要关注系统内部的表示是否**统一、稳定、可验证**。

核心原则：

1. **内部统一**：同一类数据在系统内部尽量只流通一种主表示，避免长期混用两套等价但口径不同的格式
2. **边界收口**：如果外部输入可能有多种格式，应在边界层尽早完成归一、校验或报错
3. **为演进预留载体**：当前只用到一小部分字段时，也可以先建立可扩展的公共载体，避免未来把上下文拆散到参数、全局变量或隐式状态里

当前项目中的两个典型案例：

- **路径格式**：相对路径 / 绝对路径的选择，关键不是偏好哪一种，而是选定后要把它变成系统内部稳定的数据契约
- **`ToolContext`**：当前先承载 `cwd`，但形态上是后续挂载平台标识、版本信息、状态数据等上下文的公共载体
