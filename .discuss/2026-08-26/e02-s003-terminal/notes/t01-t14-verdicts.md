...
>
> 状态：⚠️ **本文是 D01–D05 之前的早期建议，已被部分推翻，保留作推理轨迹。**
> **以 [outline.md 的 Confirmed 表](../outline.md) 为准。**
>
> 被推翻的五条（都是「调研共识」输给「本机实测」的例子，本身是教学素材）：
>
> | # | 早期建议 | 最终决策 | 推翻理由 |
> |---|---|---|---|
> | T03 | 不加 `-l`，因为「rc 成果已在 `process.env`」 | 启动时 `$SHELL -lc` 采集一次 | 前提在桌面 App / 服务 / CI / cron 下全不成立（D05-2） |
> | T05 | 保尾弃头截断 | **不截断**，超阈值只给规模 + 路径 | 截断把「上下文预算」问题当「输出长度」问题解（D02） |
> | T06 | `TERM=xterm-256color`（抄 Gemini） | `TERM=dumb` | Gemini 的理由是驱动 pty 里的 emulator，我们不做 pty（D05-10） |
> | T09 | 默认 120s 超时 + 到点杀 | **不设上限**，改「取消 / 跳过」 | 杀死不可逆，而「该不该继续等」只有人能判断（D03-1/4） |
> | T14 | 后台执行交给 S004 | **收回 S003** | 它是 S003 自己制造的问题，不能留给下一章（D04-13） |
>
> ⚠️ **五条里有三条（T05/T06/T09）是"抄竞品共识"抄错的** —— 竞品的做法要连着它的前提一起看。
